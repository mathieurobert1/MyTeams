...
void handle_response(client_t *client, char *msg) 
{
    ...
    if (code == CLIENT_EVENT_THREAD_REPLY_RECEIVED)
        client_event_thread_reply_received(parsed[1], parsed[2], parsed[3], parsed[4]);
    ...
    if (code == CLIENT_EVENT_THREAD_CREATED)
        client_event_thread_created(parsed[1], parsed[2], (long int) atoi(parsed[3]), parsed[4], parsed[5]);
    ...
    if (code == CLIENT_CHANNEL_PRINT_THREADS)
        client_channel_print_threads(parsed[1], parsed[2], (long int) atoi(parsed[3]), parsed[4], parsed[5]);
    if (code == CLIENT_THREAD_PRINT_REPLIES)
        client_thread_print_replies(parsed[1], parsed[2], (long int) atoi(parsed[3]), parsed[4]);
    if (code == CLIENT_PRIVATE_MESSAGE_PRINT_MESSAGES)
        client_private_message_print_messages(parsed[1], (long int) atoi(parsed[2]), parsed[3]);
    ...
    if (code == CLIENT_PRINT_THREAD)
        client_print_thread(parsed[1], parsed[2], (long int) atoi(parsed[3]), parsed[4], parsed[5]);
    ...
    if (code == CLIENT_PRINT_THREAD_CREATED)
        client_print_thread_created(parsed[1], parsed[2], (long int) atoi(parsed[3]), parsed[4], parsed[5]);
    if (code == CLIENT_PRINT_REPLY_CREATED)
        client_print_reply_created(parsed[1], parsed[2], (long int) atoi(parsed[3]), parsed[4]);
